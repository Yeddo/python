...
def encode_data(type_data, data):
    ...
    if 'raw' in type_data:
        ...
        print("ENCODE: HEX")
        output = binascii.hexlify(data.encode()).decode()
    elif 'oct' in type_data:
        print("ENCODE: OCT")
        output = int(binascii.hexlify(data.encode()).decode(), base=16)
        output = oct(output)[2:]
    elif 'dec' in type_data:
        print("ENCODE: DEC")
