	...
	def run(self):
		...
		while True:
			try:
				...
				s = socket.socket()
				s.connect((str(ip), int(self.port)))
				banner = s.recv(1024)
				if banner != None:
					Alive.append(str(ip) + '\n')
...
def main():
	...
	port = raw_input('What port would you like to scan?\n')
	
	# creating threads to execute tasks
	for i in range(int(threads)):
		worker = workerthread(queue, port)
	...
	f.close()
	print 'Wrote ' + str(len(Alive)) + ' hosts to ./files/targets.txt'
